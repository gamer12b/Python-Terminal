...
while True:
    ...
    if code == "cpu":
       ...
       <<< Max Frequency: {freq.max:.2f}Mhz  >>>
       <<< Min Frequency: {freq.min:.2f}Mhz  >>>
       <<< Current Frequency: {freq.current:.2f}Mhz  >>>   
    ...
    if code == "memory":
        ...
        <<< Total: {int(svmem.total / 1000000)} Mb >>> 
        <<< Available: {int(svmem.available / 1000000)} Mb >>>
        <<< Used: {int(svmem.used / 1000000)} Mb >>>
        """
        print(str(info))
    ...
    if code == "disk":
       info = f"""
        ...
        <<< Disk Total: {int(psutil.disk_usage("/").total / 1000000000)} Gigabytes >>>
        <<< Disk Usage: {int(psutil.disk_usage("/").used / 1000000000)} Gigabytes >>>
        <<< Disk Free:  {int(psutil.disk_usage("/").free / 1000000000)} Gigabytes >>>
        """
       print(str(info))
    if code == "help":
      print("""
